...
Use these guides when you already have a Dock Area profile and need to manage it in an existing dock area.

If you are learning profiles for the first time, start with
[Dock Area Profiles](../../getting-started/next-steps/dock-area-profiles-tutorial.md){ data-preview }. To create the
same profiles from BEC IPython commands, use
...
1. Create a profile in the
   ...
   tutorial.
2. [Switch between profiles](switch-dock-area-profile.md){ data-preview } during normal work.
3. [Adjust quick selection](toggle-dock-area-profile-quick-selection.md){ data-preview } so the toolbar only shows the
   profiles you use often.
4. [Restore a profile](restore-dock-area-profile-baseline.md){ data-preview } when runtime changes should be discarded.
5. [Share useful profiles](share-dock-area-profile-with-other-accounts.md){ data-preview } before deleting local copies.
